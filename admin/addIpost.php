	...
	if(in_array($fileActualExt, $allowed)){
		if($fileError ===  0 ){
			if ($fileSize < 1000000) { //10MB
				...
				header("Location: addIpost.php?uploadsuccess");
			}else{
				header("Location:addIpost.php?post=FileSizeIsMore");
			}

		}else{
			header("Location:addIpost.php?post=ErrorOnFileUpload");
		}

	}else{
		header("Location:addIpost.php?post=ThisFileCannotUpload");
	}
	//Error Handlers
	if(empty($title) || empty($body) || empty($author)) {
		header("Location:addIpost.php?post=empty");
		}else{
			//Insert User to database
			$sql ="INSERT INTO `intercollegepost`(`id`, `title`, `body`, `author`, `image`) VALUES (NULL,'$title','$body','$author','$fileNameNew')";
			mysqli_query($conn,$sql);
			header("Location:addIpost.php?post=AddedSucessfully");
		}
}else{
...
<div class="container">
	<form method="POST" action="addIpost.php" enctype="multipart/form-data">	
